...
const TradeCard = ({ trade }: { trade: TradeItem }) => (
  ...
  >
    ...
    <div className="flex-1 px-5 space-y-2">
      {/* HAVE 섹션 */}
      <div className="bg-indigo-50/50 rounded-xl border border-indigo-100 p-3.5 relative group-hover:bg-indigo-50 transition-colors">
      ...
      <div className="flex justify-center -my-3 relative z-10">
        <div className="bg-white p-1.5 rounded-full border border-slate-200 text-slate-400 shadow-sm">
          <ArrowRightLeft className="w-3.5 h-3.5" />
        </div>
      </div>

      {/* WANT 섹션 */}
      <div className="bg-pink-50/50 rounded-xl border border-pink-100 p-3.5 relative group-hover:bg-pink-50 transition-colors">
        <div className="flex items-center gap-2 mb-2.5">
    ...
    {/* 하단 액션 */}
    <div className="p-4 mt-5 bg-slate-50/80 border-t border-slate-100 flex items-center justify-between">
      <div className="flex flex-col gap-0.5">
        <div className="flex items-center gap-1.5">
            <span className="text-sm font-bold text-slate-700">{trade.author.name}</span>
            {trade.author.level === 'cafe' && (
                <span className="text-[9px] bg-green-100 text-green-700 px-1 py-0.5 rounded border border-green-200 font-medium">
                카페인증
                </span>
            )}
        </div>
        <span className="text-[10px] text-slate-400">안전한 거래를 권장합니다</span>
      </div>
...
export default function GoodsTrade() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
      ...
      {/* 1. 주의사항 (상단 배치) - 디자인 개선 */}
      <div className="bg-orange-50/80 border border-orange-200/60 rounded-2xl p-4 flex items-start gap-4 shadow-sm backdrop-blur-sm">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-100 pb-6">
        <div className="space-y-2">
            ...
            <p className="text-slate-500 pl-[60px] font-medium">
            중복 굿즈는 나누고, 필요한 굿즈는 채워보세요.
            </p>
        </div>
        <button className="bg-slate-900 hover:bg-slate-800 text-white px-6 py-3 rounded-xl font-bold text-sm transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5 flex items-center gap-2 whitespace-nowrap">
      ...
      {/* 3. 필터 바 & 컨트롤 - 디자인 고도화 */}
      <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col lg:flex-row gap-5">
        ...
        <div className="relative flex-1 min-w-[280px]">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-slate-50 border-transparent rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all placeholder:text-slate-400 text-sm text-slate-700 font-medium"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
            ...
            <button
                ...
                className={cn(
                "flex items-center gap-2.5 px-4 py-3 rounded-xl border text-sm font-bold transition-all whitespace-nowrap min-w-[140px] justify-between group",
                mainRegion !== '전체'
            ...
            >
                ...
                <MapPin className={cn("w-4 h-4", mainRegion !== '전체' ? "text-indigo-600" : "text-slate-400 group-hover:text-slate-500")} />

...
export function GoodsTrade() {
  ...
  return (
    // ✅ 전체 레이아웃: 패딩을 p-6 ~ p-8로 늘려 여유 공간 확보
    <div className="max-w-7xl mx-auto p-6 md:p-8 space-y-8 bg-transparent">
      
      {/* 1. 주의사항 (Warm Amber) */}
      <div className="bg-amber-50 border border-amber-100 rounded-[20px] p-5 flex items-start gap-3 text-sm text-amber-800 shadow-sm">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-100 pb-8">
        <div className="space-y-2">
          <h1 className="text-3xl font-extrabold text-slate-800 flex items-center gap-3">
            {/* 아이콘: Teal 그라데이션 적용 */}
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center shadow-lg shadow-teal-100">
              <RefreshCw className="w-6 h-6 text-white" />
            </div>
            굿즈 교환소
          </h1>
          <p className="text-slate-500 text-base font-medium pl-[60px]">
            중복 굿즈는 교환하고, 없는 굿즈는 채워보세요.
          </p>
        </div>

        {/* 작성 버튼: Teal Primary Color */}
      ...
      {/* 3. 필터 바 & 컨트롤 */}
      {/* rounded-3xl 적용 */}
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col lg:flex-row lg:items-center gap-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className={cn(
              ...
              mainRegion !== '전체' 
                ? "bg-teal-50 border-teal-200 text-teal-700 shadow-sm" // Active: Teal
          ...
          <button
            ...
            className={cn(
              ...
              deliveryOnly 
                ? "bg-blue-50 border-blue-200 text-blue-600 shadow-sm" // Active: Blue
                : "bg-white border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-700"
            )}
          >
        ...
        {filteredTrades.length > 0 ? (
          filteredTrades.map((trade) => (
            <div 
              key={trade.id} 
              className={cn(
                // ✅ Card: rounded-3xl 적용, Hover시 Teal Border
            ...
            >
              ...
              {/* 하단 액션 */}
              <div className="px-6 py-5 mt-2 bg-gradient-to-b from-white to-slate-50 border-t border-slate-50 flex items-center justify-between rounded-b-[24px]">
          ...
          <div className="col-span-full py-32 text-center bg-white rounded-[32px] border-2 border-dashed border-slate-200">

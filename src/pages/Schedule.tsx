        ...
        <div className="col-span-2 bg-white/70 backdrop-blur-xl rounded-2xl p-6 shadow-sm border border-purple-50 flex flex-col h-full overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-4 flex-shrink-0 px-4 pt-2">
            ...
            <div className="flex gap-2">
              <button onClick={previousMonth} className="w-9 h-9 hover:bg-purple-50 rounded-full flex items-center justify-center transition-colors">
                <ChevronLeft className="w-6 h-6 text-gray-600" />
              </button>
              <button onClick={nextMonth} className="w-9 h-9 hover:bg-purple-50 rounded-full flex items-center justify-center transition-colors">
            ...
            <div className="grid grid-cols-7 grid-rows-6 gap-3 h-full content-start">
              {calendarCells.map((day, i) => {
                ...
                const isSelected =
                  ...
                  new Date(selectedEvent.date).getMonth() === currentDate.getMonth();
                ...
                return (
                  <button
                    ...
                    disabled={!day}
                    className={`
                      w-full h-16 self-center rounded-2xl flex flex-col items-center justify-center
                      transition-all duration-300 gap-0.5
                      ${day && event
                  ...
                  >
                    {day && (
                      <>
                        <span className={`text-base ${event ? 'font-bold opacity-90' : ''}`}>{day}</span>
                        {event && <span className="text-2xl">{getEventIcon(event.type)}</span>}
                      </>
                    )}
                  </button>
                );
            ...
            {schedules?.map((event) => {
              ...
              return (
                <button
                  ...
                  className={`
                    w-full px-4 py-3 rounded-xl transition-all duration-200 text-left flex items-center gap-3
                    ${isSelected
                      ? 'bg-purple-50 border-purple-100 ring-1 ring-purple-100'
                      : 'hover:bg-white/50 border border-transparent'}
                  `}
                >
                  <div className="flex flex-col items-center justify-center min-w-[3rem] border-r pr-3 text-gray-400">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-bold truncate text-gray-600">
                      {event.title}
                    </p>

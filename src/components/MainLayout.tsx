  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-slate-900 bg-gray-50/50">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 shadow-sm">
        <div className="max-w-[1200px] mx-auto px-4 h-[72px] flex items-center justify-between"> {/* 높이 72px로 조정 */}
          
          {/* ✅ 1. 로고 크기 축소 (h-5 w-5, text-xl) */}
          ...
          {/* ✅ 2. 메뉴 우측 정렬 (ml-auto) & 모바일 가로 스크롤 복구 */}
          <nav className="flex items-center gap-2 overflow-x-auto no-scrollbar ml-auto w-auto justify-end pr-0">
            {NAV_ITEMS.map((item) => {
              const Icon = item.icon;
              const isActive = location.pathname.startsWith(item.path);
              
              // ✅ 3. 호버 반응 복구: Tailwind 유틸리티 클래스 직접 사용 (CSS 파일 의존 X)
              const themeStyles: any = {
                blue:    { activeBg: "bg-blue-50 border-blue-200 shadow-sm ring-1 ring-blue-100", iconActive: "bg-gradient-to-br from-blue-400 to-cyan-400 text-white shadow-blue-200", iconInactive: "bg-white text-blue-300 group-hover:text-blue-400", textActive: "text-blue-900", hover: "hover:bg-blue-50/50 hover:border-blue-100 hover:shadow-sm" },
                rose:    { activeBg: "bg-rose-50 border-rose-200 shadow-sm ring-1 ring-rose-100", iconActive: "bg-gradient-to-br from-rose-400 to-pink-400 text-white shadow-rose-200", iconInactive: "bg-white text-rose-300 group-hover:text-rose-400", textActive: "text-rose-900", hover: "hover:bg-rose-50/50 hover:border-rose-100 hover:shadow-sm" },
                violet:  { activeBg: "bg-violet-50 border-violet-200 shadow-sm ring-1 ring-violet-100", iconActive: "bg-gradient-to-br from-violet-400 to-purple-400 text-white shadow-violet-200", iconInactive: "bg-white text-violet-300 group-hover:text-violet-400", textActive: "text-violet-900", hover: "hover:bg-violet-50/50 hover:border-violet-100 hover:shadow-sm" },
                amber:   { activeBg: "bg-amber-50 border-amber-200 shadow-sm ring-1 ring-amber-100", iconActive: "bg-gradient-to-br from-amber-400 to-orange-400 text-white shadow-amber-200", iconInactive: "bg-white text-amber-300 group-hover:text-amber-400", textActive: "text-amber-900", hover: "hover:bg-amber-50/50 hover:border-amber-100 hover:shadow-sm" },
                emerald: { activeBg: "bg-emerald-50 border-emerald-200 shadow-sm ring-1 ring-emerald-100", iconActive: "bg-gradient-to-br from-emerald-400 to-teal-400 text-white shadow-emerald-200", iconInactive: "bg-white text-emerald-300 group-hover:text-emerald-400", textActive: "text-emerald-900", hover: "hover:bg-emerald-50/50 hover:border-emerald-100 hover:shadow-sm" },
              };
              ...
              return (
                <Link
                  ...
                  `}
                  style={{ minWidth: '130px' }} // 버튼 최소 너비 확보
                >
                  {/* 아이콘 박스 */}
                  <div 
                    className={`

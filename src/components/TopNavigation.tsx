...
const NAV_ITEMS = [
  ...
  { path: '/timeline', icon: Twitter, label: '타임라인', id: 'indigo' },ㅁimport { useState, useEffect } from 'react';
import { Link, useLocation } from 'react-router-dom';
import { Calendar, Zap, Radio, Twitter, ShoppingBag, Sparkles, Menu, X } from 'lucide-react';
        ...
        <div className="w-full h-full max-w-[1700px] mx-auto px-4 md:px-6 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-2 self-center"> 
            {NAV_ITEMS.map((item) => {
              ...
              // ✅ [수정] h-[52px], px-4
              // - 높이 52px: 상단바(64px) 안에서 위아래 6px씩 남김 (딱 좋은 간격)
              // - px-4: 좌우 패딩을 줄여서 버튼이 너무 뚱뚱해지지 않게 조절
              const baseLayout = "flex items-center gap-2.5 h-[52px] px-4 rounded-xl border transition-all duration-200 group active:scale-95 font-bold whitespace-nowrap";
    ...
    <>
      ...
      >
        ...
        <div className="w-full h-full max-w-[1700px] mx-auto px-4 md:px-6 flex items-center justify-between">
          ...
          {/* 🟧 [메뉴] 상단바를 꽉 채우는 60px 버튼 */}
          <nav className="hidden md:flex items-center gap-2 self-center"> 
            {NAV_ITEMS.map((item) => {
              const isActive = location.pathname.startsWith(item.path);
              
              // ✅ [최종 수정] h-[60px]
              // 64px(헤더) - 60px(버튼) = 4px 여백 (위 2px / 아래 2px)
              // px-6: 버튼이 아주 크므로 좌우도 넉넉하게
              const baseLayout = "flex items-center gap-3 h-[60px] px-6 rounded-2xl border transition-all duration-200 group active:scale-95 font-bold whitespace-nowrap";
